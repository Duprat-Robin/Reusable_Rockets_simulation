...
ti1 = 15; % Final time for phase 1 (s). Must be a short time (i for intermediate)
% On a paper we got 12s.
[t1, y1] = ode45(@(t, y) ascent_dynamicsODE_2(t, T(stage), y, param, gamma0), [t0 ti1], y01, options);

%2.
phase = 2;
param = [Isp(stage), Cd, A, stage, phase];

y02 = [y1(end,ix) y1(end,idx)-0.01*pi/180 y1(end,ih) y1(end,idh) y1(end,im)];

tb1 = mp(stage)*g0*Isp(stage)/T(stage); %burnout time of 1st stage (s)
tf1 = tb1; %end of the 1st stage phase
[t2, y2] = ode45(@(t, y) ascent_dynamicsODE_2(t, T(stage), y, param), [ti1 tf1], y02, options);
...
y03 = [y2(end,ix) y2(end,idx) y2(end,ih) y2(end,idh) y2(end,im)];
...
[t3, y3] = ode45(@(t, y) ascent_dynamicsODE_2(t, T(stage), y, param), [tf1 tf2], y03, options);
...
y04 = [y3(end,ix) y3(end,idx) y3(end,ih) y3(end,idh) y3(end,im)];
...
[t4, y4] = ode45(@(t, y) ascent_dynamicsODE_2(t, T(stage), y, param, acos(y3(end,idx)/(sqrt(y3(end,idx)^2+y3(end,idh)^2))), [tf2 tf3]), [tf2 tf3], y04, options);
...
figure(2); hold on;
plot(t1,acos(y1(end,idx)/(sqrt(y1(end,idx)^2+y1(end,idh)^2)))*180/pi,'r','LineWidth',2);
plot(t2,acos(y2(end,idx)/(sqrt(y2(end,idx)^2+y2(end,idh)^2)))*180/pi,'g','LineWidth',2);
plot(t3,acos(y3(end,idx)/(sqrt(y3(end,idx)^2+y3(end,idh)^2)))*180/pi,'b','LineWidth',2);
plot(t4,acos(y4(end,idx)/(sqrt(y4(end,idx)^2+y4(end,idh)^2)))*180/pi,'y','LineWidth',2);
